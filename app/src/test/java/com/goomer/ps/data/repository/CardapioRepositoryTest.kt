...
class CardapioRepositoryTest {
    ...
    @Test
    fun `getMenuItems deve mapear DTOs para Domain corretamente`() = runTest {
        // Given
        val dtoList = listOf(
            MenuItemDto(
                id = 1,
    ...
    @Test
    fun `getMenuItems deve retornar lista vazia quando não há itens`() = runTest {
        // Given
    ...
    @Test
    fun `getMenuItems deve lançar exceção quando DataSource falha`() = runTest {
        // Given
    ...
    @Test
    fun `getMenuItemById deve retornar item correto quando existe`() = runTest {
        // Given
        val dtoList = listOf(
            MenuItemDto(id = 1, name = "Pizza", description = "Deliciosa pizza", price = 29.90, imageUrl = ""),
            MenuItemDto(id = 2, name = "Hamburguer", description = "Hamburguer artesanal", price = 19.90, imageUrl = ""),
    ...
    @Test
    fun `getMenuItemById deve retornar null quando item não existe`() = runTest {
        // Given
        val dtoList = listOf(
            MenuItemDto(id = 1, name = "Pizza", description = "Deliciosa pizza", price = 29.90, imageUrl = "")
        )
    ...
    @Test
    fun `getMenuItemById deve lançar exceção quando DataSource falha`() = runTest {
        // Given
        val exception = RuntimeException("Erro ao ler arquivo")
        whenever(localDataSource.loadMenuItems()).thenThrow(exception)
    ...
    @Test
    fun `getMenuItemById deve retornar primeiro item quando há múltiplos com mesmo ID`() = runTest {
        // Given
        val dtoList = listOf(
    ...
    @Test
    fun `mapeamento deve preservar todos os campos do DTO`() = runTest {
        // Given
        val dtoWithAllFields = MenuItemDto(
            id = 42,
            name = "Teste Completo",
